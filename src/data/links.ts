export interface LinkItem {
  title: string;
  url: string;
}

export interface LinkSection {
  section: string;
  items: LinkItem[];
}

export const links: LinkSection[] = [
  {
    section: "General",
    items: [
      {
        title: "Which jobs help people the most?",
        url: "https://80000hours.org/career-guide/high-impact-jobs/",
      },
      {
        title: "The 3 kinds of non-fiction books",
        url: "https://commoncog.com/blog/the-3-kinds-of-non-fiction-book/",
      },
      {
        title: "Cities and Ambition",
        url: "http://www.paulgraham.com/cities.html",
      },
      {
        title: "How Will You Measure Your Life",
        url: "https://hbr.org/2010/07/how-will-you-measure-your-life",
      },
      {
        title: "Advice by Patrick Collison",
        url: "https://patrickcollison.com/advice",
      },
      {
        title: "Design Philosophy",
        url: "http://www.paulgraham.com/design.html",
      },
      {
        title: "Why Our IQ Levels are Higher Than Our Grandparents'",
        url: "https://www.ted.com/talks/james_flynn_why_our_iq_levels_are_higher_than_our_grandparents/discussion?utm_campaign=BeepBeepBites%20-%20Nieuwsbrief&utm_source=hs_email&utm_medium=email&_hsenc=p2ANqtz-_2hFqnJe7bosPi-VvlewtW941D69tVvFt6qM1IybJvfE20MndKH58gdlK6LNcrEfTIfKHz",
      },
      {
        title:
          "How do I form more connections in my brain so I can be a livelier conversationalist and a quicker writer?",
        url: "https://www.quora.com/How-do-I-form-more-connections-in-my-brain-so-I-can-be-a-livelier-conversationalist-and-a-quicker-writer/answer/Asher-Nitin?ch=99&share=bd3a6f38&srid=5Tf9",
      },
      {
        title: "68 Bits of Unsolicited Advice",
        url: "https://kk.org/thetechnium/68-bits-of-unsolicited-advice/",
      },
      {
        title: "Mental Models",
        url: "https://fs.blog/mental-models/#what_are_mental_models",
      },
      {
        title: "Hard work vs. Long work",
        url: "https://seths.blog/2011/05/hard-work-vs-long-work/",
      },
      {
        title: "Boring is fun",
        url: "https://jamesstuber.com/boring-is-fun/?utm_campaign=Ali%20Abdaal&utm_medium=email&utm_source=Revue%20newsletter",
      },
      {
        title: "Cognitive biases warping your perception of reality",
        url: "https://www.weforum.org/agenda/2018/12/24-cognitive-biases-that-are-warping-your-perception-of-reality",
      },
      {
        title: "To Get Good, Go After The Metagame",
        url: "https://commoncog.com/blog/to-get-good-go-after-the-metagame/",
      },
    ],
  },
  {
    section: "Code",
    items: [
      {
        title: "The Missing Semester of Your CS Education",
        url: "https://missing.csail.mit.edu/",
      },
      {
        title: "Top 10 pieces of advice for rookie software engineers",
        url: "https://blog.gds-gov.tech/top-10-pieces-of-advice-for-rookie-software-engineer-f1a2b38f1d56",
      },
      {
        title: "Google Engineering Practices",
        url: "https://google.github.io/eng-practices/",
      },
      {
        title: "Tech Interview Handbook",
        url: "https://yangshun.github.io/tech-interview-handbook/",
      },
      {
        title: "Why is processing a sorted array faster than processing an unsorted array?",
        url: "https://stackoverflow.com/questions/11227809/why-is-processing-a-sorted-array-faster-than-processing-an-unsorted-array",
      },
      {
        title: "Developer Roadmap",
        url: "https://github.com/kamranahmedse/developer-roadmap",
      },
      {
        title: "Manager's Playbook",
        url: "https://github.com/ksindi/managers-playbook",
      },
      {
        title: "Never attribute to stupidity that which is adequately explained by opportunity cost",
        url: "https://erikbern.com/2020/03/10/never-attribute-to-stupidity-that-which-is-adequately-explained-by-opportunity-cost.html",
      },
      {
        title: "Rules of thumb for a 1x developer",
        url: "https://muldoon.cloud/programming/2020/04/17/programming-rules-thumb.html",
      },
      {
        title: "Why is it recommended to have empty line in the end of a source file?",
        url: "https://stackoverflow.com/questions/2287967/why-is-it-recommended-to-have-empty-line-in-the-end-of-a-source-file",
      },
      {
        title: "The most successful devs share more than they take",
        url: "https://stackoverflow.blog/2020/05/14/the-most-successful-developers-share-more-than-they-take/?cb=1",
      },
      {
        title: "Little Bobby Tables",
        url: "https://www.explainxkcd.com/wiki/index.php/Little_Bobby_Tables",
      },
      {
        title: "Systems design for advanced beginners",
        url: "https://robertheaton.com/2020/04/06/systems-design-for-advanced-beginners/",
      },
      {
        title: "Essays on programming",
        url: "https://www.benkuhn.net/progessays/",
      },
      {
        title: "Unix vs Linux",
        url: "https://www.youtube.com/watch?v=jowCUo_UGts",
      },
      {
        title: "Rob Pike's 5 Rules of Programming",
        url: "http://users.ece.utexas.edu/~adnan/pike.html",
      },
    ],
  },
];
